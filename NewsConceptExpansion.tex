...
1) Web sites to be crawled. Google allows to specify a list of web domains and subdomains where documents can be retrieved from. This reduces the scope of the search task and, depending on the characteristics of the considered sources, influence the nature of the retrieved items: from big online newspapers to user generated content. At the same time, google allows to prioritize searching over those whitelists while still considering the  whole indexed Web. Based on this, in our study we considered five possible values for this parameter:
a) AllGoogle: search over the whole set of Web pages indexed by Google.
b) Newspaper whitelist. A set of 10 internationals English speaking newspapers chosen from \footnote{\fontsize{8pt}{1em}\selectfont  \url{ http://en.wikipedia.org/wiki/List_of_newspapers_in_the_world_by_circulation}}  
c) Lilia's whitelist. A set of 3 international newspapers used in [Lilia] while building the ground truth used in Section~\ref{sec:evaluation}.
d) Newspaper whiteList + Google. Prioritize content in Newspaper whitelist but still consider other sites.
e) Lilia's whitelist + AllGoogle: Prioritize content in Lilia's whitelist but still consider other sites.

2) Temporal dimension. This variable allows to keep only those documents which are temporally close from the day where the newscast was published. Assuming that the news item is fresh enough, this date of publication will also be close to the day the even depicted actually took place. Taking that date and extending the  $t+t_{e}$. The reason why we expand the original event period is because documents concerning a news event are not always published during the time of the action is taking place but some hours or days after or before. The value of $w$ depends on many factors such as the nature of the event itself (whether it is a brief appearance in a media, or part of a longer story with more repercussion) or the kind of documents the search engine is indexing (from very deep and elaborated documents that need time to be published, to short post quickly generated by users). In this study we have considered two possible values for this dimension: 2 weeks and one week temporal window.

3) In addition, Google Custom Search Engine makes possible to filter result according to the Schema.org types: 2 values: . Based on the study. 
[NoFilter(Default), Person\&OrganizationFiltered]

That makes $5 * 2 * 2 = 20$ different runs. We haver run experiments in order to see which configuration optimizes the expansion algorithm.
...
As you probably saw in the first round of results, there were many non-pure Named Entities which were not considered on Lilia’s survey and therefore don't appear in the GT. Those concepts were dropping the scores to a shameful 0.035 average NDCG for the TFIDF strategy, the most affected one.

Yesterday I worked with Giuseppe on a plan to filter out those entities. We foresaw three different filtering approaches for getting rid of those undesired NEs:
- F1 (Filter by type). Keeping only nerd:Person, nerd:Location, and nerd:Organization entities.
- F2 (Filter by confidence). Drop entities with confidence score under e.g. first quarter of the distribution
- F3 (Filter instances ) Proper names are normally uppercased, keep only those.

I executed different runs by applying those filters over the original annotations (and their corresponding combinations, named: [F1 - F2 - F3 - F1_F2 - F1_F3 - F2_F3 - F1_F2_F3] ). You can find in the repository:

New Annotations: https://github.com/jluisred/NewsConceptExpansion/tree/master/data/video/firstSurvey/annotationsSecondPhase
Results: https://github.com/jluisred/NewsConceptExpansion/tree/master/data/video/firstSurvey/result 
Updated scores: https://github.com/jluisred/NewsConceptExpansion/tree/master/data/video/firstSurvey/evaluation 


Just created an excel which summarizes the new results in terms of NDCG. Improvements in precision at 10,  recall at 10 and average precision are even more evident.  https://github.com/jluisred/NewsConceptExpansion/blob/master/data/video/firstSurvey/evaluation/Filters_NDCG.csv . Some quick outcomes: 

1) The problem with non strict Named Entities in TFIDF strategy is clearly alleviated for all filters/Combinations. TF strategy has generally improved as well, but in a lower scale. 
...
4) Best Filter for TFIDF strategy is the combination F1_F3 
5) I combined the results for both strategies via a) simple multiplication and b) their average value, see rows 4th and 5th. In both cases, the combination F1_F3 seems to perform the best.
